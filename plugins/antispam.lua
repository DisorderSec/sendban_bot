            ...
            if not roles.is_admin_cached(msg) then
                ...
                if warns_received >= max_allowed then
                    ...
                    if action == 'ban' then
                        ...
                        hammer_text = _('banido seu lixinho')
                    elseif action == 'kick' then
                        hammer_funct = api.kickUser
                        hammer_text = _('kickado otário')
                    end
...
local function changeWarnsNumber(chat_id, action)
    ...
    if current == 1 and action == 'dim' then
        return _("Você não pode ir mais baixo")
    elseif current == 7 and action == 'raise' then
        return _("Você não pode ir mais alto")
    else
        local new
        if action == 'dim' then
        ...
        return _("Novo valor: %d"):format(new)
    end
end
...
local function  get_alert_text(key)
    if key == 'links' then
        return _("Permitir/proibir telegram.me links")
    elseif key == 'forwards' then
        return _("Permitir/proibir as mensagens encaminhadas dos canais")
    elseif key == 'warns' then
        return _("Defina quantas vezes o bot deve avisar o usuário antes de chutá-lo/proibi-lo")
    else
        return 'description'
    end
end
...
local function doKeyboard_antispam(chat_id)
    ...
    local humanizations = {
        ...
        ['forwards'] = _('Mensagens de canais')
    }
...
function plugin.onCallbackQuery(msg, blocks)
	...
	if blocks[1] == 'alert' then
	    ...
	    if not roles.is_admin_cached(chat_id, msg.from.id) then
	    	api.answerCallbackQuery(msg.cb_id, _("Você não é mais um administrador"))
	    else
