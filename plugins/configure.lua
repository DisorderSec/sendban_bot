...
local function do_keyboard_config(chat_id)
    local keyboard = {
        inline_keyboard = {
            ...
            {{text = _("🌈 Midias"), callback_data = 'config:media:'..chat_id}},
...
function plugin.onCallbackQuery(msg, blocks)
    ...
    local keyboard = do_keyboard_config(chat_id)
    api.editMessageText(msg.chat.id, msg.message_id, _("_Altere as configurações navegando no teclado_"), true, keyboard)
end
