...
function plugin.onTextMessage(msg, blocks)
	...
	if blocks[1] == 'init' then
		...
		api.sendReply(msg, '*Bot recarregado!*\n_'..n_plugins..' plugins ativados_', true)
	end
	...
	if blocks[1] == 'save' then
		db:bgsave()
		api.sendMessage(msg.chat.id, 'Redis atualizado', true)
	end
    if blocks[1] == 'stats' then
	    ...
	    text = text..'- *uptime*: `from '..(os.date("%c", start_timestamp))..' (GMT+3)`\n'
	...
	if blocks[1] == 'unblock' then
		...
		if response == 1 then
			...
			text = id..' já estava desbloqueado'
		end
		api.sendReply(msg, text)
	end
	...
	if blocks[1] == 'download' then
		if not msg.reply then
			api.sendMessage(msg.chat.id, 'Responda a uma mensagem')
		else
			local type
			local file_id
			local file_name
	...
	if blocks[1] == 'update' then
		...
		api.sendReply(msg, 'Feito. Substituído '..n..' chave(s)')
	end
