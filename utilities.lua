...
function misc.get_sm_error_string(code)
	local descriptions = {
		[109] = _("O link foi formatado incorretamente. Verifique o texto entre colchetes -> \\[]()"),
		[141] = _("O link foi formatado incorretamente. Verifique o texto entre colchetes -> \\[]()"),
		[142] = _("O link foi formatado incorretamente. Verifique o texto entre colchetes -> \\[]()"),
		[112] = _("Este texto quebra o markdown.\n"
					.. "Mais informações sobre o uso adequado do markdown "
					.. "[here](https://telegram.me/GroupButler_ch/46)."),
		[118] = _('Esta mensagem é muito longa. Comprimento máximo permitido pelo Telegram: 4000 caracteres')
	}
	
	return descriptions[code] or _("Markdown error")
end
...
function misc.get_media_id(msg)
	...
	elseif msg.sticker then
		...
		return false, 'A mensagem não tem uma mídia file_id'
	end
end
...
function misc.migrate_chat_info(old, new, on_request)
	...
	if on_request then
		api.sendReply(msg, 'Deve ser feito')
	end
end
...
function misc.to_supergroup(msg)
	...
	if done then
		misc.remGroup(old, true, 'para supergrupo')
		api.sendMessage(new, '(_service notification: migração para grupo executada_)', true)
	end
end
...
function misc.getSettings(chat_id)
    ...
    local message = _("Configs atuais *do grupo*:\n\n")
			.. _("*Linguagem*: %s\n"):format(config.available_languages[lang])
        
    --build the message
	local strings = {
		Welcome = _("Mensagem de bem-vindo"),
		Goodbye = _("Mensagem de adeus"),
		Extra = _("Extra"),
		Flood = _("Anti-flood"),
		Antibot = _("Ban bots"),
